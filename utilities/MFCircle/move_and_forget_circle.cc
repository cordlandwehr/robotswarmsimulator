...
void simulator::random_walk(std::vector< simulator::node >& nodes, const std::size_t& steps)
{
  ...
  for (i = 0; i < n; i++) {
	...
	nodes[i].long_range_link_age = 0;
      } else if (simulator::flip_coin(id)) {
...
inline std::size_t simulator::ring_distance(const std::vector<simulator::node>& nodes, const std::size_t& a, const std::size_t& b)
{
  std::size_t n = nodes.size();
...
std::vector< size_t > simulator::calculate_length_distribution(const std::vector< simulator::node >& nodes)
{
  ...
  for (std::size_t i = 0; i < n; i++) {
    std::size_t distance = ring_distance(nodes, nodes[i].id, nodes[i].long_range_link);
    result[distance]++;
  }
  return result;
}
