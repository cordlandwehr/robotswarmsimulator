...
class GFSPTree {
	...
	public:
		...
		gdiam_real brute_diameter(int a_lo, int a_hi, int b_lo, int b_hi,
		...
		gdiam_real brute_diameter(int a_lo, int a_hi, int b_lo, int b_hi,
				GPointPair & diam, const gdiam_point dir) const {
			double max_dist;
		...
		gdiam_real brute_diameter(const gdiam_point * a_lo,
				const gdiam_point * a_hi, const gdiam_point * b_lo,
				const gdiam_point * b_hi, GPointPair & diam) const {
			double max_dist;
		...
		gdiam_real brute_diameter(const gdiam_point * a_lo,
				const gdiam_point * a_hi, const gdiam_point * b_lo,
				const gdiam_point * b_hi, GPointPair & diam,
		...
		int split_array(gdiam_point * left, gdiam_point * right, int dim,
		...
		void split_node(GFSPTreeNode * node) {
			...
			left_size = split_array(node->p_pnt_left, node->p_pnt_right, dim,
					split_val);
			if (left_size <= 0.0) {
			...
			node->left = build_node(node->p_pnt_left, node->p_pnt_left
					+ left_size - 1);
...
gdiam_real bbox_proj_dist(const GBBox & bb1, const GBBox & bb2,
	...
	public:
		...
		void init(GFSPTreeNode * _left, GFSPTreeNode * _right,
			...
			max_diam = max(max(bbox_proj_dist(_left->getBBox(),
					_right->getBBox(), proj_dir), bbox_proj_dist(
					_left->getBBox(), _left->getBBox(), proj_dir)),
					bbox_proj_dist(_right->getBBox(), _right->getBBox(),
							proj_dir));
		...
		double directDiameter(const GFSPTree & tree, GPointPair & diam) const {
			return tree.brute_diameter(left->ptr_pnt_left(),
					left->ptr_pnt_right(), right->ptr_pnt_left(),
					right->ptr_pnt_right(), diam);
		}

		double directDiameter(const GFSPTree & tree, GPointPair & diam,
				const gdiam_point dir) const {
			return tree.brute_diameter(left->ptr_pnt_left(),
					left->ptr_pnt_right(), right->ptr_pnt_left(),
					right->ptr_pnt_right(), diam, dir);
		}
...
class GTreeDiamAlg {
	...
	public:
		...
		void addPairHeap(g_heap_pairs_p & heap, GFSPTreeNode * left,
				GFSPTreeNode * right);
		void addPairHeap(g_heap_pairs_p & heap, GFSPTreeNode * left,
				GFSPTreeNode * right, gdiam_point proj, GFSPPair & father);
		void split_pair(GFSPPair & pair, g_heap_pairs_p & heap, double eps);
		void split_pair_proj(GFSPPair & pair, g_heap_pairs_p & heap,
...
void computeExtremePair(const gdiam_point * arr, int size, int dim,
		GPointPair & pp) {
	pp.init(arr[0]);
...
void GTreeDiamAlg::compute_by_heap(double eps) {
	...
	computeExtremePair(tree.getPoints(), points_num,
			root->getBBox().getLongestDim(), pair_diam);
...
void GTreeDiamAlg::compute_by_heap_proj(double eps, gdiam_point proj) {
	g_heap_pairs_p heap;
	int heap_limit, heap_delta;
	...
	computeExtremePair(tree.getPoints(), points_num,
			root->getBBox().getLongestDim(), pair_diam_x);
...
void GTreeDiamAlg::addPairHeap(g_heap_pairs_p & heap, GFSPTreeNode * left,
		GFSPTreeNode * right) {
...
void GTreeDiamAlg::split_pair_proj(GFSPPair & pair, g_heap_pairs_p & heap,
		double eps, gdiam_point proj) {
	...
	if (f_is_left_splitable && f_is_right_splitable) {
		addPairHeap(heap, pair.get_left()->get_left(),
		...
		if (pair.get_left() != pair.get_right())
			addPairHeap(heap, pair.get_left()->get_right(),
					pair.get_right()->get_left(), proj, pair);
		addPairHeap(heap, pair.get_left()->get_right(),
				pair.get_right()->get_right(), proj, pair);
		return;
	...
	if (f_is_left_splitable) {
		addPairHeap(heap, pair.get_left()->get_left(), pair.get_right(), proj,
				pair);
		addPairHeap(heap, pair.get_left()->get_right(), pair.get_right(), proj,
				pair);
		return;
	}
	if (f_is_right_splitable) {
		addPairHeap(heap, pair.get_left(), pair.get_right()->get_left(), proj,
				pair);
		addPairHeap(heap, pair.get_left(), pair.get_right()->get_right(), proj,
				pair);
		return;
	}
}

void GTreeDiamAlg::split_pair(GFSPPair & pair, g_heap_pairs_p & heap,
		double eps) {
	bool f_is_left_splitable, f_is_right_splitable;
...
gdiam_bbox gdiam_approx_const_mvbb(gdiam_point * start, int size,
		gdiam_real eps, GBBox * p_ap_bbox) {
	//gdiam_real  diam;
	...
	if ((pnt_length(dir) == 0.0) || (pnt_length(dir_2) < 1e-6) || (pnt_length(
			dir_3) < 1e-6)) {
		gdiam_generate_orthonormal_base(dir, dir_2, dir_3);
		pnt_dump(dir);
		pnt_dump(dir_2);
...
void gdiam_generate_orthonormal_base(gdiam_point in, gdiam_point out1,
		gdiam_point out2) {
...
inline ldouble Area(const point2d & a, const point2d & b, const point2d & c) {
...
inline int AreaSign(const point2d & a, const point2d & b, const point2d & c) {
	ldouble area;

	area = a.x * b.y - a.y * b.x + a.y * c.x - a.x * c.y + b.x * c.y - c.x
			* b.y;
...
inline bool Left(const point2d & a, const point2d & b, const point2d & c) {
	return AreaSign(a, b, c) > 0;
}

inline bool Left_colinear(const point2d & a, const point2d & b,
		const point2d & c) {
	return AreaSign(a, b, c) >= 0;
}
...
static void remove_duplicate(vec_point_2d & in, point2d_ptr ptr, int start) {
	int dest;
...
class MinAreaRectangle {
	...
	public:
		...
		void compute_edge_dir(int u) {
			...
			if ((u > 0) && (angles[u] < angles[u - 1]) && eq_real(angles[u],
					angles[u - 1]))
				angles[u] = angles[u - 1];
		...
		int find_vertex(int start_vertex, double trg_angle) {
			...
			while (!f_found) {
				...
				if (prev_angle <= curr_angle)
					f_found = ((prev_angle < trg_angle) && (trg_angle
							<= curr_angle));
				else
					f_found = ((prev_angle < trg_angle) || (trg_angle
							<= curr_angle));
		...
		/* Computing the intersection point of two lines, each given by a
		 point and slope */
		void compute_crossing(int a_ind, double a_angle, int b_ind,
				double b_angle, gdiam_real & x, gdiam_real & y) {
		...
		void get_bbox(int a_ind, gdiam_real a_angle, int b_ind,
				gdiam_real b_angle, int c_ind, gdiam_real c_angle, int d_ind,
				gdiam_real d_angle, bbox_2d_info & bbox, gdiam_real & area) {
			/*
			 printf( "get_bbox: (%d, %g,\n"
			 "%d, %g\n"
			 "%d, %g\n"
			...
			compute_crossing(a_ind, a_angle, b_ind, b_angle,
					bbox.vertices[0][0], bbox.vertices[0][1]);
			compute_crossing(b_ind, b_angle, c_ind, c_angle,
					bbox.vertices[1][0], bbox.vertices[1][1]);
			compute_crossing(c_ind, c_angle, d_ind, d_angle,
					bbox.vertices[2][0], bbox.vertices[2][1]);
			compute_crossing(d_ind, d_angle, a_ind, a_angle,
					bbox.vertices[3][0], bbox.vertices[3][1]);
		...
		void get_angles(int ind, gdiam_real & angle_1, gdiam_real & angle_2,
				gdiam_real & angle_3) {
			double angle;
		...
		void compute_min_bbox_inner(bbox_2d_info & min_bbox,
			...
			angles
					= (gdiam_real *) malloc(sizeof(gdiam_real)
							* (int) ch.size());
			assert( angles != NULL );
			...
			//printf( "_angles[ 0 ]: %g\n", angles[ 0 ] );
			get_bbox(0, angles[0], s, ang1, v, ang2, t, ang3, min_bbox,
					min_area);
			//min_bbox.dump();
			...
			for (u = 1; u < (int) ch.size(); u++) {
				...
				get_bbox(u, angles[u], s, ang1, v, ang2, t, ang3, tmp_bbox,
						tmp_area);
				//tmp_bbox.dump();
				//printf( "tmp_area: %g\n", tmp_area );
		...
		void compute_min_bbox(vec_point_2d & points, bbox_2d_info & min_bbox,
				gdiam_real & min_area) {
...
static void construct_base_inner(gdiam_point pnt1, gdiam_point pnt2,
		gdiam_point pnt3) {
	pnt_normalize(pnt1);
	pnt_normalize(pnt2);
	pnt_normalize(pnt3);
...
class ProjPointSet {
	...
	public:
		...
		void compute() {
			...
			if ((!(fabs(pnt_dot_prod(base_proj, out_1)) < 1e-6)) || (!(fabs(
					pnt_dot_prod(base_proj, out_2)) < 1e-6)) || (!(fabs(
					pnt_dot_prod(out_1, out_2)) < 1e-6))) {
				printf("should be all close to zero: %g, %g, %g\n",
						pnt_dot_prod(base_proj, out_1), pnt_dot_prod(base_proj,
								out_2), pnt_dot_prod(out_1, out_2));
				pnt_dump(base_proj);
				pnt_dump(out_1);
				pnt_dump(out_2);
...
static void try_direction(gdiam_bbox & bb, gdiam_bbox & bb_out,
		gdiam_point * start, int size, int x_coef, int y_coef, int z_coef) {
	gdiam_bbox bb_new;
...
static void register_point(gdiam_point pnt, gdiam_point * tops,
		gdiam_point * bottoms, int grid_size, gdiam_bbox & bb) {
