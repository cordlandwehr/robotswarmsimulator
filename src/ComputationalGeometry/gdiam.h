...
inline void pnt_copy(gdiam_point_t dest, gdiam_point_t src) {
	dest[0] = src[0];
	dest[1] = src[1];
	dest[2] = src[2];
}
...
inline gdiam_real pnt_dot_prod(gdiam_point_cnt a, gdiam_point_cnt b) {
	return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

inline void pnt_cross_prod(const gdiam_point a, const gdiam_point b,
...
inline gdiam_real pnt_distance_2d(gdiam_point_2d p, gdiam_point_2d q) {
...
inline void pnt_init(gdiam_point pnt, gdiam_real x, gdiam_real y, gdiam_real z) {
	pnt[0] = x;
	pnt[1] = y;
	pnt[2] = z;
}

inline void pnt_init_normalize(gdiam_point pnt, gdiam_real x, gdiam_real y,
		gdiam_real z) {
	pnt[0] = x;
	pnt[1] = y;
	pnt[2] = z;
...
inline bool pnt_isEqual(const gdiam_point p, const gdiam_point q) {
	// Assuming here the GDIAM_DIM == 3 !!!!
	return ((p[0] == q[0]) && (p[1] == q[1]) && (p[2] == q[2]));
}

inline void pnt_scale_and_add(gdiam_point dest, gdiam_real coef,
		gdiam_point_cnt vec) {
	dest[0] += coef * vec[0];
	dest[1] += coef * vec[1];
	dest[2] += coef * vec[2];
...
class gdiam_bbox {
	...
	public:
		...
		void combine(gdiam_point out, double a_coef, double b_coef,
				double c_coef) {
			for (int ind = 0; ind < GDIAM_DIM; ind++)
				out[ind] = a_coef * dir_1[ind] + b_coef * dir_2[ind] + c_coef
						* dir_3[ind];
		}
		...
		void dump() const {
			...
			printf("prods: %g, %g, %g\n", pnt_dot_prod(dir_1, dir_2),
					pnt_dot_prod(dir_1, dir_3), pnt_dot_prod(dir_2, dir_3));
		...
		void init(const gdiam_point _dir_1, const gdiam_point _dir_2,
			...
			if ((!(fabs(pnt_dot_prod(dir_1, dir_2)) < 1e-6)) || (!(fabs(
					pnt_dot_prod(dir_1, dir_3)) < 1e-6)) || (!(fabs(
					pnt_dot_prod(dir_2, dir_3)) < 1e-6))) {
				printf("should be all close to zero: %g, %g, %g\n",
						pnt_dot_prod(dir_1, dir_2), pnt_dot_prod(dir_1, dir_3),
						pnt_dot_prod(dir_2, dir_3));
				pnt_dump(_dir_1);
				pnt_dump(_dir_2);
				pnt_dump(_dir_3);
