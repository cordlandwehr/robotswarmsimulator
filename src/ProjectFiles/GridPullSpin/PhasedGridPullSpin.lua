--[[
	PhasedGridPullSpin
	Variant of GridPullSpin (see below).
...
zero = Vector3d(0,0,0)
time = 0
R = nil          -- maximum distance of a robot to the target point in initial arrangement (computed in main)
...
--[[
---- Returns true if there is a robot standing at robot_position and this robot will perform a pull towards position.
--]]
function robot_pulling_at_position(robot_position, position)
...
--[[
---- Returns true iff this is an outer robot (not same definition as in paper: don't check whether robot can move, this
---- is done implicitly by collision handling).
--]]
function is_outer_robot()
	local target_point = target_point()
	local x_pull = Vector3d(sign(target_point.x), 0, 0)
	local y_pull = Vector3d(0, sign(target_point.y), 0)
	
	if (target_point.x == 0) then
		return not position_occupied(Vector3d(1,0,0)) and not position_occupied(Vector3d(-1,0,0)) and not position_occupied(zero-y_pull)
	end
	
	if (target_point.y == 0) then
		return not position_occupied(Vector3d(0,1,0)) and not position_occupied(Vector3d(0,-1,0)) and not position_occupied(zero-x_pull)
	end
	
	return not position_occupied(zero-x_pull) and not position_occupied(zero-y_pull)
end


--[[
---- Computes the maximum distance of a robot to the target point
--]]
function compute_maximum_targetdistance()
	local target_point = target_point()
	local max_distance = l1norm(target_point)

	local visible_robots = View.get_visible_robots()
	for ignore, robot in pairs(visible_robots) do
		local robot_position = View.get_position(robot)
		local curr_distance = l1norm(robot_position-target_point)
		if (curr_distance > max_distance) then
...
--[[
---- Implementation of the GridPullSpin algorithm.
--]]
function GridPullSpin()
...
function main()
	...
	if (R == nil) then
		R = compute_maximum_targetdistance()
	end
	
	if (time < R) then
		View.add_position_request(x_pull)
	elseif (time < 2*R) then
		View.add_position_request(y_pull)
	elseif (is_outer_robot()) then
		GridPullSpin()
	end
