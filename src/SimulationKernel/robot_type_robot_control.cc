...
RobotTypeRobotControl::RobotTypeRobotControl(const boost::array<boost::shared_ptr<AbstractViewFactory>,kRobotTypeCount>& view_factories, std::size_t history_length, const WorldInformation& initial_world_information)
...
boost::shared_ptr<boost::array<boost::shared_ptr<View>,kRobotTypeCount> > RobotTypeRobotControl::create_views(const WorldInformation& world_information) {
...
void RobotTypeRobotControl::update(const WorldInformation& world_information) {
...
void RobotTypeRobotControl::compute_view(Robot& robot) {
	...
	robot.set_view(boost::weak_ptr<View>((*view_buffer_.back())[index]));
}
