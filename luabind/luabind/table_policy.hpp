...
struct table_policy: conversion_policy<N> {
	...
	struct apply {
		static const bool IsPtr = luabind::detail::is_nonconst_pointer<T>::value || luabind::detail::is_const_pointer<T>::value;
...
namespace luabind {
template<int N>
detail::policy_cons<detail::table_policy<N, false>, detail::null_type> copy_table(LUABIND_PLACEHOLDER_ARG(N)) {
	return detail::policy_cons<detail::table_policy<N, false>, detail::null_type>();
}

template<int N>
detail::policy_cons<detail::table_policy<N, true>, detail::null_type> copy_table_assoc(LUABIND_PLACEHOLDER_ARG(N)) {
	return detail::policy_cons<detail::table_policy<N, true>, detail::null_type>();
}
}
